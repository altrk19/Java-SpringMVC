...
public class ProductDAOImpl implements ProductDAO {
	...
	@Override
	public Product getProductById(String productId) {                //id'si verilen ürünü döner
		// eski yöntem
		/*
		...
		*/
		...
		//java8 ile gelen predicate kullanýrsak
	...
	public List<Product> getProductsByCategory(String categoryName) {
		...
		for(Product product:listOfProducts) {
			if(product.getCategory().equalsIgnoreCase(categoryName)) {							//kategorisi ayný olan ürünleri listeye ekle sonra listeyi view sýnýfýna dön
				productsByCategory.add(product);
		}
		return productsByCategory;*/
		
		//java8 ile yazdýk. listeyi filtreledik
		Predicate<Product> predicate = (Product product) -> product.getCategory().equalsIgnoreCase(categoryName);
	...
	@Override
	public List<Product> getProductsByBrands(List<String> brands) {             //brand:istekte gelen markalar
		...
		*/
		...
		List<Product> productsByBrands = listOfProducts.stream().filter(predicate).collect(Collectors.toList());		
		return productsByBrands;
	}
